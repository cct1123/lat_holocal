...
class FarFieldSim():
    ...
    def __init__(self, ap_field, msmt_geo, rx):
        ...
        radius = 3.0 # [m]
        # radius = 100.0 # [m]
        validindex = np.where(self.x_ap*self.x_ap+self.y_ap*self.y_ap<=radius*radius)
        self.x_ap = self.x_ap[validindex]
        self.y_ap = self.y_ap[validindex]
        self.z_ap = self.z_ap[validindex]

        # Propagation vector of the sample points (tan_og)
        # self.k_x = ap_field[12, :]
        # self.k_y = ap_field[13, :]
        self.k_z = ap_field[14, :][validindex]

        self.pathl = ap_field[15, :][validindex] / 1e3  # Path length convert to meters
        ...
        self.Fcomplex = ampl * np.exp(self.ima * self.pathl * self.k)
    ...
    def find_beam(self, i_ang, j_ang):
        ...
        # field_far_integrand = self.Fcomplex * np.exp(self.ima * self.k * r) * z_dot_rhat  / (r * self.pathl)
        # field_far = np.sum(field_far_integrand) * self.k / (2*np.pi*1j) / len(field_far_integrand)
        # out2 = field_far
    ...
    def output(self):
        if self.N_scan > 55: # a threshold to use parallelism (it's a empirical number tested with Ryzen 5 3600)
            ...
            result = []
            for ii in range(-self.N_scan, self.N_scan, 1):
                for jj in range(-self.N_scan, self.N_scan, 1):
