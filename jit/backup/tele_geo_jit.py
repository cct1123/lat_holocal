...
def z1(x, y):
    # amp = np.zeros_like(x)
    amp = 0.0
    for ii in nb.prange(7):
        for jj in nb.prange(7):
...
def m1_into_tele(x, y, z, th1=initialize_telescope_geometry.th_1):
...
@jit(nopython=True, fastmath=True)
def m2_into_tele(x, y, z, th2=initialize_telescope_geometry.th2):
...
@jit(nopython=True, fastmath=True)
def tele_into_m1(x, y, z, th1=initialize_telescope_geometry.th_1):
    z += 3600.0
    y += 7200.0
...
@jit(nopython=True, fastmath=True)
def tele_into_m2(x, y, z, th2=initialize_telescope_geometry.th2):
    y += 4800.0 + 7200.0
    x_temp = x
...
if __name__ == "__main__":
    ...
    # x = np.linspace(-2000.0, 2000.0, 200)
    # y = np.linspace(-2000.0, 2000.0, 200)
    x=2
    y=3
    @jit(nopython=True, parallel=True, fastmath=True)
    def z1(x, y):
    ...
    def z1_np(x, y):
        # amp = np.zeros_like(x)
        xx = (x/R_N) ** np.arange(7)
        yy = (y/R_N) ** np.arange(7)
        # xmesh, ymesh = np.meshgrid(xx, yy)
        # return np.sum(a1*xmesh.T*ymesh.T)
        # xmesh = xx.repeat(7).reshape(7, 7).transpose()
        xmesh = np.column_stack((xx, xx, xx, xx, xx, xx, xx))
        ymesh = np.vstack((yy, yy, yy, yy, yy, yy, yy))
        return np.sum(a1*xmesh*ymesh)
    ...
    for i in range(loop_num):
        ...
        result = z1(x, y)
    t2 = time()
    print(f'Function {z1.__name__!r} (looped {loop_num} times) executed in {(t2-t1):.8f}s')
